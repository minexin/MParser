...
Contract 1.7 adds the shared advanced statistics, dense-linear-algebra,
...
Contract 1.11 added the `sum` reduction identity to `BuiltinTypedLowering`.
Contract 1.12 adds the `prod` and `mean` reduction identities. These identities
remain declarative: the VM is the semantic authority, while an eligible Dense
Typed region may consume them only after proving the descriptor is the
unshadowed default builtin and validating argument, numeric-class, shape, and
resource guards. Other reductions remain ordinary registry calls until they
receive an independently tested lowering identity.
...
preserving lowering is available. Contract 1.14 adds the repository-owned CSC
