...
foreach(required_variable IN ITEMS
    ...
    if(NOT DEFINED ${required_variable} OR
       "${${required_variable}}" STREQUAL "")
        message(FATAL_ERROR
            "Missing v1.3 public-contract variable: ${required_variable}")
    endif()
endforeach()
...
elseif(NOT CONTRACT_STATE STREQUAL "archived")
    message(FATAL_ERROR
        "Invalid v1.3 contract state: ${CONTRACT_STATE}")
endif()
...
function(validate_contract_artifact description expected_path)
    ...
    if(NOT contract_is_current AND
       artifact_path STREQUAL "docs/machine-result-v1.schema.json")
        set(artifact "${PROJECT_ROOT}/tests/public_contract/protocol/1.1/machine-result.schema.json")
    endif()
...
require_json("v1.3" "candidate milestone" release milestone)
...
require_json("17" "builtin source contract minor"
    builtin_source_contract minor)
require_json("328" "builtin descriptor count"
    builtin_source_contract descriptor_count)
require_json("330" "builtin registered-name count"
...
validate_contract_artifact("CLI 1.0 contract" "docs/cli-contract-v1.json"
    cli contract)
validate_live_contract_artifact("C API 1.3 header" "include/mparser/c_api.h"
    c_api header)
validate_contract_artifact("C ABI generation-2 snapshot"
    "tests/public_contract/c_abi/2.1/c_api_snapshot.h" c_abi snapshot)
validate_contract_artifact("C ABI generation-2 symbols"
    "tests/c_api_generation2_revision1_symbols.txt" c_abi symbols)
validate_live_contract_artifact("C++ API 1.3 header"
    "include/mparser/cpp_api.hpp" cpp_api header)
validate_contract_artifact("C++ API 1.3 snapshot"
    "tests/public_contract/cpp_api/1.3/mparser/cpp_api.hpp"
    cpp_api snapshot_header)
validate_contract_artifact("C++ API 1.3 C dependency snapshot"
    "tests/public_contract/cpp_api/1.3/mparser/c_api.h"
...
validate_contract_artifact("builtin 1.17 catalog"
    "tests/public_contract/builtin/1.17/default_catalog.json"
...
if(contract_is_current)
    ...
    file(READ
        "${PROJECT_ROOT}/src/mparser/runtime/builtins/builtin_registry.h"
        builtin_header)
    foreach(version_pair IN ITEMS
            "kBuiltinSourceContractMajor;1"
            "kBuiltinSourceContractMinor;17")
        list(GET version_pair 0 constant)
        list(GET version_pair 1 expected)
        string(REGEX MATCH
            "${constant}[ \t]*=[ \t]*([0-9]+)"
...
file(STRINGS
    "${PROJECT_ROOT}/tests/c_api_generation2_revision1_symbols.txt"
    symbols)
list(FILTER symbols EXCLUDE REGEX "^[ \t]*$")
list(LENGTH symbols symbol_count)
if(NOT symbol_count EQUAL 117)
    message(FATAL_ERROR
        "C ABI generation-2 symbol manifest has ${symbol_count} entries")
endif()

file(READ
    "${PROJECT_ROOT}/tests/public_contract/builtin/1.17/default_catalog.json"
    builtin_catalog)
...
if(NOT descriptor_count EQUAL 328 OR
   NOT catalog_major EQUAL 1 OR NOT catalog_minor EQUAL 17)
    message(FATAL_ERROR "Builtin 1.17 catalog identity changed")
endif()
...
endforeach()
if(NOT registered_name_count EQUAL 330)
    message(FATAL_ERROR
...
if(contract_is_current)
    ...
    foreach(required_text IN ITEMS
            "set(MParser_BUILTIN_SOURCE_CONTRACT_MINOR \"17\")"
...
message(STATUS
    ...
    "source contract 1.17, 328 descriptors and 330 names")
